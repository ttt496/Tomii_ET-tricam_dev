...
class SampleRecord:
    ...
    window_size: Optional[Tuple[int, int]] = None  # (width, height) from session_meta.json


def _load_session_window_size(session_path: Path) -> Optional[Tuple[int, int]]:
    """Load window size from session_meta.json if available."""
...
def load_eye_crop_samples(
    ...
    # セッションごとの画面サイズをキャッシュ
    session_window_sizes: Dict[str, Optional[Tuple[int, int]]] = {}
    ...
    for (_session, _frame), data in groups.items():
        ...
        # セッションの画面サイズを取得（キャッシュから）
        session = data["session"]
        if session not in session_window_sizes:
            # セッションパスを構築して画面サイズを読み込む
            session_path = base_dir / session if base_dir else Path(session)
            session_window_sizes[session] = _load_session_window_size(session_path)
        ...
        samples.append(
            SampleRecord(
                ...
                window_size=session_window_sizes[session],
            )
        )
...
class TriCamEyeDataset(Dataset):
    def __init__(
        ...
        frame_size: Tuple[int, int],  # デフォルト/フォールバック用
        patch_size: Tuple[int, int],
        image_format: str = "png",
    ):
        self.samples = list(samples)
        self.camera_ids = list(camera_ids)
        self.default_frame_width, self.default_frame_height = frame_size
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        for eye_idx, (cam_id, eye_side) in enumerate(self.eye_order):
            ...
            if record.center:
                # サンプルごとの画面サイズを使用（なければデフォルト値）
                window_size = sample.window_size
                if window_size is not None:
                    frame_width, frame_height = window_size
                else:
                    frame_width, frame_height = self.default_frame_width, self.default_frame_height
                
                u = float(record.center[0]) / float(frame_width)
                v = float(record.center[1]) / float(frame_height)
